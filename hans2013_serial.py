...
class inv_cc_amp:
    ...
    def setup(self,f0,lowf,highf,ptukey,dinfo):
        ...
        self.nstart = int(self.ngpib/2) + 1
        ...
        self.Green = np.zeros((3,3,ntot_omost,ntot_omost,self.nom_nneg), dtype='complex')
        ...
        for i in range(1,self.nom_nneg):
            ...
            if self.pss[i] < thresh:
                ...
                if scalar:
                    # self.Green[:,:,i] = ssp.hankel1(0,self.omega[i]*r/self.c) * 1j * 0.25
                    self.Green[2,2,:,:,i] = ssp.hankel1(0,self.omega[i]*r/self.c) * 1j * 0.25
                elif elastic:
...
#			deltapos = np.square(origdist_rp - self.dist_rp)
        ...
        while todo_syn:
            for k in range(self.nrecs-1):
                for j in range(k+1,self.nrecs):
                    ...
                    Grec_j = self.Green[comp_dic['z'],comp_dic[p_comp],nyst[j]:nyfin[j],nxst[j]:nxfin[j],:]
                    Grec_k = self.Green[comp_dic['z'],comp_dic[q_comp],nyst[k]:nyfin[k],nxst[k]:nxfin[k],:]
                    # ARJUN: note coordinate transformation here!! From position vector "r" to "r - r_alpha"

                    # f_inv = np.conj(self.Green[nyst[k]:nyfin[k],nxst[k]:nxfin[k],1:self.nom_nneg]) * self.Green[nyst[j]:nyfin[j],nxst[j]:nxfin[j],1:self.nom_nneg]
                    ...
                    # SYNTHETIC DATA CASE
                        ...
                        Grec_j = self.Green[comp_dic['z'],comp_dic[p_comp],nyst_om[j]:nyfin_om[j],nxst_om[j]:nxfin_om[j],:]
                        Grec_k = self.Green[comp_dic['z'],comp_dic[q_comp],nyst_om[k]:nyfin_om[k],nxst_om[k]:nxfin_om[k],:]
                        # f_true = np.conj(self.Green[nyst_om[k]:nyfin_om[k],nxst_om[k]:nxfin_om[k],1:self.nom_nneg]) * self.Green[nyst_om[j]:nyfin_om[j],nxst_om[j]:nxfin_om[j],1:self.nom_nneg]
    ...
    # Computing source kernels for positive and negative branches
        ...
        GrecA = self.Green[comp_dic['z'],comp_dic[p_comp],nyst1:nyfin1,nxst1:nxfin1,:]
        GrecB = self.Green[comp_dic['z'],comp_dic[q_comp],nyst2:nyfin2,nxst2:nxfin2,:]
...
if __name__ == '__main__':
    ...
    sig_char.dt = 0.2 #0.05 #0.2
    sig_char.nsam = 250 #400 #250
    sig_char.cf = 0.3 #2.0 #0.3
    sig_char.lf = None
    sig_char.hf = None
